...
class KeyInfoWindow:
    def __init__(self, root, private_key, public_key, shared_key, peer_public_key, source):
        ...
        tk.Label(self.window, text=str(private_key)).grid(row=0, column=1, sticky="w")
        ...
        tk.Label(self.window, text=str(public_key)).grid(row=1, column=1, sticky="w")

        # Hiển thị khóa chia sẻ (Shared Key)
        tk.Label(self.window, text="Khóa chia sẻ (Shared Key):").grid(row=2, column=0, sticky="w")
        tk.Label(self.window, text=str(shared_key)).grid(row=2, column=1, sticky="w")
        ...
        tk.Label(self.window, text=str(peer_public_key)).grid(row=3, column=1, sticky="w")

        # Nút đóng cửa sổ
        tk.Button(self.window, text="Đóng", command=self.window.destroy).grid(row=4, column=0, columnspan=2)

        # Thêm label để hiển thị chữ ký
        self.signature_label = tk.Label(self.window, text="Chữ ký (Signature):")
        self.signature_label.grid(row=5, column=0, sticky="w")
        self.signature_value = tk.Label(self.window, text="Chưa có chữ ký")
        self.signature_value.grid(row=5, column=1, sticky="w")
